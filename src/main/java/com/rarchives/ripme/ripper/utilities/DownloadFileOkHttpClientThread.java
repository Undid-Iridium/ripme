 ...
 * and other goodies.
 */
public class DownloadFileOkHttpClientThread implements Runnable {
    private static final Logger logger = LogManager.getLogger(DownloadFileOkHttpClientThread.class);
    ...
    public DownloadFileOkHttpClientThread(URL url, File saveAs, AbstractRipper observer, Boolean getFileExtFromMIME, OkHttpClient client) {
        super();
        this.url = url;
        this.saveAs = saveAs;
    ...
    public void run() {
        ...
        do {
            ...
            try {
                ...
                String HTTP_USER_AGENT = "Mozilla/5.0 (iPhone; CPU iPhone OS 15_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/15.0 Mobile/15E148 Safari/604.1";
                ...
                if (response.body().contentLength() == 503 && urlToDownload.getHost().endsWith("imgur.com")) {
                ...
                if (observer.useByteProgessBar()) {
                    bytesTotal = (int) response.body().contentLength();
                ...
                int bytesRead;
                boolean shouldSkipFileDownload = response.body().contentLength() / 1000000 >= 10 && AbstractRipper.isThisATest();
